...
Page({  
  ...
  /**
* merchantmoves_2_2处理函数
* 数据绑定  data-index="{{index}}" 
* 数据取值  var index = e.currentTarget.dataset.index; var cata = this.data.list_cata_list[index];
*/
merchantmoves_2_2_click:function(e){

},

/**
* merchantmoves_7_7处理函数
* 数据绑定  data-index="{{index}}" 
* 数据取值  var index = e.currentTarget.dataset.index; var cata = this.data.list_cata_list[index];
*/
merchantmoves_7_7_click:function(e){
...
* picker处理函数
*/
list_hangye_Change:function(e){
   console.log('picker发送选择改变，携带值为', e.detail.value);
   this.setData({
     index_list_hangye: e.detail.value
   })
            this.dataRefresh("list_hangye_click");
},

/**
* textarea处理函数
*/
content_jiyi_input:function(e){
    this.setData({  
      content_jiyi: e.detail.value
    })  
...
* 数据取值  var index = e.currentTarget.dataset.index; var cata = this.data.list_cata_list[index];
*/
btn_xiayi_27_click:function(e){

   wx.navigateTo({
   url:'/pages/merchantmovest/merchantmovest'
  })
            this.dataRefresh("btn_xiayi_click");
},
  ...
  dataRefresh:function(_action) {
	...
	//上传页面中的变量
        //定义变量---start
	list_hangye: that.data.index_list_hangye,
content_jiyi: that.data.content_jiyi,
      ...
      success: function (res) {//后端返回数据
        ...
        if(_action=="init"){
		that.setData({
		array_list_hangye: tmp.picker_list_hangye,
	...
	that.setData({
		index_list_hangye: tmp.list_hangye,
content_jiyi: tmp.content_jiyi,
content_jiyi: tmp.content_jiyi,
